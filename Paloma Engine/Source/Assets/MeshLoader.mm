...
Mesh *loadFromFile(MTL::Device *device, const char *path) {
    @autoreleasepool {
        ...
        mdlDesc.attributes[3] = [[MDLVertexAttribute alloc]
            initWithName:MDLVertexAttributeTextureCoordinate
                  format:MDLVertexFormatFloat2
                  offset:44
             bufferIndex:0];

        mdlDesc.layouts[0] = [[MDLVertexBufferLayout alloc] initWithStride:52];

        // Load MDLAsset with our descriptor
        MDLAsset *asset = [[MDLAsset alloc] initWithURL:url
                                       vertexDescriptor:mdlDesc
