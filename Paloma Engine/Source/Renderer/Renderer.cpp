...
bool Renderer::init(MTL::Device *device) {
    ...
    _environmentBuffer = _context.device()->newBuffer(
                                                      sizeof(EnvironmentData), MTL::ResourceStorageModeShared);
    _environmentBuffer->setLabel(
                                 NS::String::string("EnvironmentBuffer", NS::UTF8StringEncoding));
...
// --Frame Management --
void Renderer::beginFrame() { _context.beginFrame(); }

void Renderer::endFrame(CA::MetalDrawable *drawable,
                        MTL::ResidencySet *layerResidency) {
    _context.endFrame(drawable, layerResidency);
}

// --Scene --
void Renderer::setScene(Scene *scene) { _currentScene = scene; }
...
void Renderer::renderSkybox() {
    ...
    _currentEncoder->drawPrimitives(MTL::PrimitiveTypeTriangle, NS::UInteger(0),
                                    NS::UInteger(3));
...
// --Render--
void Renderer::render(MTL4::RenderPassDescriptor *renderPass,
                      CA::MetalDrawable *drawable,
                      MTL::ResidencySet *layerResidency, float width,
    ...
    memcpy(_uniformBuffers[frameIndex]->contents(), &uniforms,
           sizeof(FrameUniforms));
    _context.argumentTable()->setAddress(
                                         _uniformBuffers[frameIndex]->gpuAddress(), BufferIndexUniforms);
