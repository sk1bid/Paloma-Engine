...
void FlyCamera::update(float deltaTime, const InputManager& input) {
    ...
    if (input.isKeyPressed(kVK_Shift)) speed *= 3.0f;
    
    if (input.isKeyPressed(kVK_ANSI_W)) position += fwd * speed;
    if (input.isKeyPressed(kVK_ANSI_S)) position -= fwd * speed;
    if (input.isKeyPressed(kVK_ANSI_A)) position -= rgt * speed;
    if (input.isKeyPressed(kVK_ANSI_D)) position += rgt * speed;
    if (input.isKeyPressed(kVK_ANSI_E)) position.y += speed;
    if (input.isKeyPressed(kVK_ANSI_Q)) position.y -= speed;
}
