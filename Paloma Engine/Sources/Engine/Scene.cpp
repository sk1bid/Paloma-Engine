    ...
    for (NS::UInteger i = 0; i < allObjects->count(); ++i) {
        ...
        if (is_kind_of<MDL::Mesh>(pObject, mdlMeshClass)) {
            ...
            for (NS::UInteger j = 0; j < attributes->count(); ++j) {
                MDL::VertexAttribute* attr = attributes->object<MDL::VertexAttribute>(j);
            ...
            auto vertexDescriptor = NS::TransferPtr(MDL::VertexDescriptor::alloc()->init());
            // === 2. Настройка атрибутов (аналог: vertexDescriptor.vertexAttributes[0]...) ===
            
            // Attr 0: Position
            {
                auto attr = vertexDescriptor->attributes()->object<MDL::VertexAttribute>(0);
            ...
            // Attr 1: Normal
            {
                auto attr = vertexDescriptor->attributes()->object<MDL::VertexAttribute>(1);
            ...
            // Attr 2: Tangent
            {
                auto attr = vertexDescriptor->attributes()->object<MDL::VertexAttribute>(2);
            ...
            // Attr 3: TexCoord 1
            {
                auto attr = vertexDescriptor->attributes()->object<MDL::VertexAttribute>(3);
            ...
            if (hasMultipleUVs) {
                auto attr = vertexDescriptor->attributes()->object<MDL::VertexAttribute>(4);
                attr->setName(MDL::VertexAttributeTextureCoordinate);
                attr->setFormat(MDL::VertexFormatFloat2);
                attr->setOffset(56);
                attr->setBufferIndex(0);
            }
            // === 3. Настройка Layout Stride (аналог: vertexDescriptor.bufferLayouts[0].stride = ...) ===
            {
                auto layout = vertexDescriptor->layouts()->object<MDL::VertexBufferLayout>(0);
            ...
            mdlMesh->addOrthTanBasisForTextureCoordinateAttributeNamed(
                                                                       MDL::VertexAttributeTextureCoordinate,
                                                                       MDL::VertexAttributeNormal,
        ...
        if (parentObject) {
            ...
            if (it != entityMap.end()) {
                ...
                printf("Logic error: Entity parent not found in map (load order issue?)\n");
            }
        } else {
