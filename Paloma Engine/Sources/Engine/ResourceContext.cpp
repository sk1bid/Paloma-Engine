...
ResourceContext::ResourceContext(MTL::Device *pDevice) : _pDevice(pDevice) {
    ...
    _dataTextureOptions = NS::RetainPtr(NS::Dictionary::dictionary(valsData, keysData, 3));
    
    // Setup Color Texture Options
    auto valSRGBTrue = NS::Number::number(true);
    NS::Object *valsColor[] = {valSRGBTrue, valMipsTrue, valStorage};
    
    _colorTextureOptions = NS::RetainPtr(NS::Dictionary::dictionary(valsColor, keysData, 3));
...
std::shared_ptr<Mesh> ResourceContext::convert(MDL::Mesh *mdlMesh) {
    ...
    // 1:1 аналог Swift: let mtkMesh = try MTKMesh(mesh: mdlMesh, device: device)
    MTK::Mesh *mtkMesh = MTK::Mesh::alloc()->init(mdlMesh, _pDevice, &pError);
    
    if (!mtkMesh) {
        std::cerr << "Failed to create MTKMesh: "
                  << (pError ? pError->localizedDescription()->cString(NS::UTF8StringEncoding) : "Unknown error")
                  << std::endl;
        return nullptr;
    }
    ...
    // 1:1 аналог Swift: mtkMesh.vertexBuffers.map { ... }
    for (NS::UInteger i = 0; i < mtkBuffers->count(); ++i) {
        ...
        NS::String* label = NS::String::string("Vertex Attributes", NS::UTF8StringEncoding);
        mtlBuffer->setLabel(label);
        
        // Важно: retain, т.к. mtkMesh мы не храним, буферы могут умереть
        resources.push_back(NS::RetainPtr(mtlBuffer));
        vertexBuffers.push_back({mtlBuffer, mtkBuffer->offset(), mtkBuffer->length()});
    }
    ...
    // 1:1 аналог Swift: mtkMesh.submeshes.enumerated().map { ... }
    for (NS::UInteger i = 0; i < mtkSubmeshes->count(); ++i) {
        ...
        submeshes.push_back(Submesh(mtkSubmesh->primitiveType(),
    ...
    std::string name = nameObj ? nameObj->cString(NS::UTF8StringEncoding) : "Mesh";
    
    // Берем дескриптор из MTKMesh!
