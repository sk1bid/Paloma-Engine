...
void Metal4Renderer::makeResources() {
  ...
  auto *pEnvPathNS = pBundle->pathForResource(
      NS::String::string("kloppenheim_06_4k", NS::UTF8StringEncoding),
  ...
  ImageBasedLight::generateImageBasedLight(
      ...
      [this](ImageBasedLight *pLight, NS::Error *pError) {
        ...
        // Ensure we look at the target regardless
        _flyCamera.lookAt({0, 0, 0});
...
//     const vector_float3 orbitCenter = {0.0f, 7.0f, 0.0f};
//
//     const vector_float3 cameraPosition = {
//         orbitRadius * sinf(_time * 0.5f),
//         8.0f,
//         orbitRadius * cosf(_time * 0.5f)
//     };
//
//     const vector_float3 up = {0.0f, 1.0f, 0.0f};
//     matrix_float4x4 viewMatrix = matrix_look_at_right_hand(cameraPosition,
//     orbitCenter, up);
//
...
void Metal4Renderer::updateCamera(float deltaTime) {

  _flyCamera.update(deltaTime, InputManager::shared());
  InputManager::shared().resetMouseDelta();

  matrix_float4x4 viewMatrix = _flyCamera.viewMatrix();
  matrix_float4x4 worldMatrix = simd_inverse(viewMatrix);
  _camera.setTransform(worldMatrix);
}

void Metal4Renderer::updateScene(float deltaTime) {
  if (!_pScene)
    return;

  _pScene->rootEntity->visitHierarchy([this](Entity *pEntity) {
    if (dynamic_cast<ModelEntity *>(pEntity)) {
      // Smooth rotation based on time (0.5 radians per second)
      float angle = _time * 0.5f;
      // Rotate around Y axis
      pEntity->transform = matrix4x4_rotation(angle, 0, 1, 0);
    }
  });
}
...
void Metal4Renderer::drawInMTKView(MTK::View *pView) {
  ...
  if (!_iblReady) {
    if (_pScene && _pScene->pLightingEnvironment &&
        _pScene->pLightingEnvironment->readyEvent) {

      if (_pScene->pLightingEnvironment->readyEvent->signaledValue() >= 1) {
        _iblReady = true;
      } else {
        return;
      }
  ...
  if (_frameIndex >= kMaxFramesInFlight) {
    const auto valueToWait = _frameIndex - kMaxFramesInFlight + 1;
...
void Metal4Renderer::configure(MTK::View *pView) {
  ...
  pView->setClearColor(MTL::ClearColor(0.1f, 0.1f, 0.1f, 1.0f));
