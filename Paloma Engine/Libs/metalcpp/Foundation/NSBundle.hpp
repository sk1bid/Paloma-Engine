...
public:
    ...
    class String* pathForResource(const class String* name, const class String* ext);
...
// 1. Оберните определение селектора в правильные namespace
namespace NS { namespace Private { namespace Selector {
    _NS_PRIVATE_DEF_SEL(pathForResource_ofType_, "pathForResource:ofType:");
}}}
// 2. Реализация метода (как и раньше, но теперь селектор будет найден)
_NS_INLINE NS::String* NS::Bundle::pathForResource(const NS::String* name, const NS::String* ext)
{
    return Object::sendMessage<String*>(this, _NS_PRIVATE_SEL(pathForResource_ofType_), name, ext);
}
